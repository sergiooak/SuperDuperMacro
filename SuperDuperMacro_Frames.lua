...
f = CreateFrame("Frame", "sdm_mainFrame", nil, UIParent and "BackdropTemplate")
...
f:SetScript("OnShow", function(self)
    PlaySound(839) -- igCharacterInfoOpen / IG_CHARACTER_INFO_OPEN
    sdm_UpdateList()
end)
f:SetScript("OnHide", function(self)
    PlaySound(840) -- igCharacterInfoClose / IG_CHARACTER_INFO_CLOSE
...
f = CreateFrame("Button", "sdm_linkToMacroFrame", sdm_mainFrame, "BackdropTemplate")
...
f:SetScript("OnClick", function()
    sdm_ShowUsage(sdm_macros[sdm_currentEdit])
end)
...
sdm_SetTooltip(f,
    "Did you know that character-specific items can belong to multiple characters? Click this button to claim this item for your current character.")
...
f = CreateFrame("Button", "sdm_bodyBackground", sdm_mainFrame, "BackdropTemplate")
...
f = CreateFrame("Frame", "sdm_listBackground", sdm_mainFrame, "BackdropTemplate")
...
fs = f:CreateFontString("sdm_macroLimitText", "OVERLAY", "GameTooltipTextSmall")
...
sdm_SetTooltip(f,
    "This limit only applies to standard macros and Super Duper button macros.  If you're running out of room, try storing some infrequently used Button Macros as Floating Macros using the 'Save As...' button.  To do this with standard macros, you will have to upgrade them first.")
...
f = CreateFrame("Slider", "sdm_iconSizeSlider", sdm_mainFrame, "BackdropTemplate")
...
f = CreateFrame("Frame", "sdm_changeIconFrame", sdm_mainFrame, UIParent and "BackdropTemplate")
f:Hide()
f:SetToplevel(false)
f:SetSize(293, 60)
...
f = CreateFrame("Frame", "sdm_newFrame", sdm_mainFrame, UIParent and "BackdropTemplate")
f:SetFrameStrata("DIALOG")
f:Hide()
f:SetWidth(280)
f:SetHeight(145)
...
f:SetScript("OnHide", function()
    UIFrameFlashStop(sdm_createMacroButton_flash)
end)
sdm_AddToExclusiveGroup(f, "centerwindows")
...
function sdm_FancifyNewRadioButton(button, text, colorCode)
    fs = button:CreateFontString(button:GetName() .. "Text", "ARTWORK", "GameTooltipTextSmall")
...
f:SetChecked(1)
sdm_SetTooltip(f,
    "Button macros can be placed on your action bars. However, you can only have a limited number of these. They can also be called with a slash command.")

f = sdm_CreateRadioButton("sdm_floatingRadio", "NewType", sdm_newFrame, sdm_buttonRadio, "Floating Macro", "f")
sdm_SetTooltip(f,
    "You can make as many floating macros as you like, but they cannot be placed on your action bars. They can called in other macros via a slash command.")
...
sdm_SetTooltip(f,
    "Folders can contain macros, scripts, or other folders. Alt-click on a folder for options and instructions.")
...
sdm_SetTooltip(f,
    "Selecting 'Character-specific' will make this available only to select characters. Use the 'Claim' and 'Disown' buttons to enable or disable it for individual characters.")
...
f:SetScript("OnEnterPressed", function()
    sdm_createMacroButton:Click()
end)
fs = f:CreateFontString(sdm_newMacroNameText, "ARTWORK", "GameFontNormal")
fs:SetPoint("RIGHT", f, "LEFT", -5, 0)
fs:SetText("Name: ")
sdm_SetTooltip(f,
    "Some characters (such as spaces) are not allowed in Super Duper macro names.  If you would like spaces (or nothing) to show on the button text, use the 'Change Name/Icon' button after you have created the macro and select the 'Different name on button' option.")
...
f = CreateFrame("Frame", "sdm_sendReceiveFrame", sdm_mainFrame, "BackdropTemplate")
f:Hide()
f:SetWidth(256)
f:SetHeight(394)
...
fs = f:CreateFontString("sdm_sendInstructionText", "ARTWORK", "GameFontNormalCenter")
fs:SetText("Send current selected macro to:")
...
f = CreateFrame("Frame", "sdm_sendBarParent", sdm_sendReceiveFrame, "BackdropTemplate")
f:SetWidth(200)
f:SetHeight(30)
...
f = CreateFrame("Frame", "sdm_receiveBarParent", sdm_sendReceiveFrame, "BackdropTemplate")
f:SetWidth(200)
f:SetHeight(30)
...
f =
    sdm_CreateRadioButton("sdm_sendBattlegroundRadio", "SendTo", sdm_sendReceiveFrame, sdm_sendRaidRadio, "Battleground")

f = sdm_CreateRadioButton("sdm_sendGuildRadio", "SendTo", sdm_sendReceiveFrame, sdm_sendBattlegroundRadio, "Guild")

f = sdm_CreateRadioButton("sdm_sendArbitraryRadio", "SendTo", sdm_sendReceiveFrame, sdm_sendGuildRadio,
...
f =
    sdm_CreateRadioButton("sdm_receiveArbitraryRadio", "ReceiveFrom", sdm_sendReceiveFrame, nil, "A specific character:")
...
f = sdm_CreateRadioButton("sdm_receiveAnyoneRadio", "ReceiveFrom", sdm_sendReceiveFrame, sdm_receiveTargetRadio,
    "Anyone")
...
function sdm_CreateDefaultMacroFrameButtons()
    ...
    f = CreateFrame("Button", "$parent_linkToSDM", MacroFrame, "BackdropTemplate")
    f:SetWidth(linkButtonSize)
    f:SetHeight(linkButtonSize)
    f:SetPoint("TOPLEFT", 287, -17)
    f:SetScript("OnClick", function()
