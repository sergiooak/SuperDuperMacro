...
function sdm_About()
    print(sdm_printPrefix .. 'by hypehuman and tweaked by spiralofhope. Version ' .. sdm_version ..
              '. Check for updates at https://github.com/spiralofhope/SuperDuperMacro/releases')
end
...
function sdm_UpdateList()
    if not sdm_mainFrame:IsShown() then
        return
    end
    local f
    ...
    for i, mTab in ipairs(sorted) do
        ...
        listItem.icon:SetPoint('LEFT', sdm_iconSize * (sdm_iconSpacing + offsets[i] * (sdm_iconSpacing + 1)) +
            (sdm_iconSize - listItem.icon:GetWidth()) / 2, 0)
...
function sdm_ChangeIconOkayed()
    ...
    if (mTab.type == 'b' or mTab.type == 'f') and sdm_ContainsIllegalChars(nameInputted, true) then
        return
    end
...
function sdm_SaveConfirmationBox(postponed)
    if (not sdm_currentEdit) or sdm_macros[sdm_currentEdit].type == 'c' or sdm_macros[sdm_currentEdit].text ==
        sdm_bodyBox:GetText() then
        RunScript(postponed)
    else
        sdm_bodyBox:ClearFocus()
    ...
    if sdm_UsedByThisChar(mTab) then
        if mTab.type == 'b' then
            print(sdm_printPrefix ..
                      'To run this macro, drag the button from the list and place it on your action bar, or use ' ..
                      string.format("%q", '/click sdb_' .. mTab.name) .. ' (case-sensitive).')
        elseif mTab.type == 'f' then
            print(sdm_printPrefix .. 'To run this macro, use ' .. string.format("%q", '/click sdf_' .. mTab.name) ..
                      ' (case-sensitive).')
        elseif mTab.type == 's' then
            print(sdm_printPrefix .. 'To run this script, use ' .. string.format("%q", '/sdm run ' .. mTab.name) ..
                      ' or use the function sdm_RunScript(' .. string.format("%q", mTab.name) .. ') (case-sensitive).')
        end
    else
...
function sdm_DefaultMacroFrameLoaded()
    ...
    hooksecurefunc('MacroFrame_Update',
        function() -- This function prevents the user from messing with macros created by SDM.
            local selectedIsSDM = nil
            local globalTab = (MacroFrame.macroBase == 0) -- Is this the global tab or the character-specific tab?
            for i, v in pairs(sdm_macros) do
                if v.type == 'b' and sdm_UsedByThisChar(v) and
                    ((globalTab and v.characters == nil) or ((not globalTab) and v.characters)) then -- if this item is supposed to have a macro in this tab
...
sdm_unusedListItems = {}
sdm_listItems = {}
sdm_unusedListItems[true] = {}
sdm_unusedListItems[false] = {}
