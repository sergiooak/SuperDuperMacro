...
function sdm_OnUpdate(gself, elapsed) -- used for sending macros
    ...
    if self.TimeSinceLastUpdate > sdm_updateInterval then
        ...
        SendAddonMessage(sdm_msgPrefix, sdm_sending.command .. sdm_sending.lines[sdm_sending.i], sdm_sending.channel,
            sdm_sending.target)
...
function sdm_RegisterMessages()
    ...
    if not success then
        print(sdm_printPrefix ..
                  'You have too many addon prefixes registered. SDM will not be able to send or receive macros.')
    end
end
...
function sdm_SendMacro(mTab, chan, tar)
    ...
    local textToSend = (sdm_version .. ',' .. sdm_minVersion .. ',' .. mTab.type .. ',' .. tostring(mTab.icon) .. ',' ..
                           tostring(mTab.characters) .. ',' .. mTab.name:len() .. ',' .. mTab.text:len() .. ',' ..
                           mTab.name .. mTab.text)
...
function sdm_ReceiveLine(line, send1)
    ...
    if sdm_receiving.first and send1 then -- this is the first line
        sdm_receiving.nameAndText, sdm_receiving.textLen, sdm_receiving.playerNameLen, sdm_receiving.perCharacter, sdm_receiving.icon, sdm_receiving.type, sdm_receiving.minVersion, sdm_receiving.sendersVersion =
            sdm_SplitString(line, ',', 7)
        ...
        if sdm_CompareVersions(sdm_receiving.sendersVersion, sdm_minVersion) == 2 or
            sdm_CompareVersions(sdm_version, sdm_receiving.minVersion) == 2 then
            print(sdm_printPrefix .. 'You failed to recieve the macro due to a version incompatibility.')
            SendAddonMessage(sdm_msgPrefix, sdm_msgCommands.ReceivingFailed .. 'Incompatible Versions,' .. sdm_version,
                'WHISPER', sdm_receiving.playerName)
            sdm_EndReceiving("|cffff0000Failed|r")
            return
        else
            SendAddonMessage(sdm_msgPrefix, sdm_msgCommands.Receiving .. sdm_version, 'WHISPER',
                sdm_receiving.playerName)
        end
    ...
    if currLen == (sdm_receiving.playerNameLen + sdm_receiving.textLen) then
        ...
        sdm_receiving.text = sdm_receiving.nameAndText:sub(sdm_receiving.playerNameLen + 1,
            sdm_receiving.playerNameLen + sdm_receiving.textLen)
    end
end
...
function sdm_CancelReceive()
    if sdm_receiving.playerName ~= '<ANYONE>' then
        SendAddonMessage(sdm_msgPrefix, sdm_msgCommands.ReceivingFailed .. 'Cancelled,' .. sdm_version, 'WHISPER',
            sdm_receiving.playerName)
    end
...
function sdm_ReceiveButtonClicked()
    ...
    if ((not sender) or sender == '' or sender == sdm_thisChar.name) then
        return
    end
